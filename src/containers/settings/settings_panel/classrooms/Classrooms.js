...
function Classrooms(props) {
  const {classrooms, AddClassrooms, DeleteClassrooms} = props;
    ...
    <div className='Classrooms'>
      ...
      <div className='add_classroom'>
        <InputGroup className='item'>
          ...
          <Input placeholder="Aa" onChange={changeNumber} type='number' value={number} />
        </InputGroup>
        <InputGroup className='item'>
          <InputGroupAddon addonType="prepend">
            <InputGroupText>count of students</InputGroupText>
          </InputGroupAddon>
          <Input placeholder="Aa" onChange={changeCount} type='number' value={count} />
        </InputGroup>
        <Button className='item' color="primary" onClick={AddClassroom}>Add</Button>{' '}
      </div>

      <div className='classroom_table' style={{maxHeight:'calc(100vh - 300px)',overflow:'auto'}}>
        <Table hover>
          <thead>
            <tr>
              <th>№ Classroom</th>
            ...
            {classrooms.map(item=>{
              return(
                <tr>
                  ...
                  <td><img className='times_solid' src={times_solid} alt="delete" onClick={()=>DeleteClassrooms(item.classroom_number)} /></td>
                </tr>
              )
            })} 
          </tbody>
